...
class Lookup:
    ...
    def __setup(self, filename, **kwargs):
        ...
        self.__default = {
            'NS'        :   kwargs.get('NS', min(self.__DATA['NS'])),
            ...
            'ID_NFINS'  :   kwargs.get('ID_NFINS', None),
    ...
    def look_up(self, out, **kwargs):
        ...
        kwargs = {k.upper(): v for k, v in kwargs.items()} # convert kwargs to upper
        defaultdict = {k:self.__default.get(k) for k in ['NS', 'VGS', 'VDS', 'VSB', 'METHOD']}
    ...
    def _SimpleLK(self, outkeys, varkeys, vararg, pars, **ipkwargs):
        ...
        Args:
            ...
            varkeys: unused
            pars: dict containing NS, VGS, VDS and VSB data
        Output:
        ...
        points = (self.__DATA['NS'], self.__DATA['VGS'], self.__DATA['VDS'],\
            self.__DATA['VSB'])
        xi_mesh = np.array(np.meshgrid(pars['NS'], pars['VGS'], pars['VDS'], pars['VSB'], indexing='ij'))
        xi = np.rollaxis(xi_mesh, 0, 5)
        xi = xi.reshape(int(xi_mesh.size/4), 4)

        output = interpn(points, ydata, xi, **ipkwargs).reshape(len(np.atleast_1d(pars['NS'])), \
    ...
    def _RatioVRatioLK(self, outkeys, varkeys, vararg, pars, **ipkwargs):
        ...
        x = interpn(points, xdata, xi, **ipkwargs).reshape(len(np.atleast_1d(pars['NS'])), \
            len(np.atleast_1d(pars['VGS'])), len(np.atleast_1d(pars['VDS'])),\
                 len(np.atleast_1d(pars['VSB'])))
        
        y = interpn(points, ydata, xi, **ipkwargs).reshape(len(np.atleast_1d(pars['NS'])), \
            len(np.atleast_1d(pars['VGS'])), len(np.atleast_1d(pars['VDS'])),\
                 len(np.atleast_1d(pars['VSB'])))
    ...
    def look_upVGS(self, **kwargs):
        ...
        Args:
            pars: dict containing NS, VGB, GM_ID and ID_W, VDS, VSB and METHOD
        Output:
            output: 1-d numpy array
        """
        kwargs = {k.upper(): v for k, v in kwargs.items()} # convert kwargs to upper
        defaultdict = {k:self.__default.get(k) for k in ['NS', 'VDS', 'VDB', 'VGB', 'GM_ID', 'ID_NFINS', 'VSB', 'METHOD']}
        ...
        if pars['ID_NFINS'] is not None:
            ratio_string = 'ID_NFINS'
            ratio_data = pars['ID_NFINS']
        ...
        # In usage mode (1), the inputs to the function are GM_ID (or ID/W), NS, 
        # VDS and VSB
        ...
        if mode == 1:
            VGS = self['VGS']
            ratio = self.look_up(ratio_string, VGS = VGS, VDS=pars['VDS'], VSB=pars['VSB'], NS=pars['NS'])
            ...
            VGS = pars['VGB'] - VSB
            VDS = pars['VDB'] - VSB
            ratio = np.array([self.look_up(ratio_string, VGS=VGS[i], VDS=VDS[i], VSB=VSB[i], NS=pars['NS']).item() for i in range(len(VGS))])
            idx = ~np.isnan(ratio)
            ratio = ratio[idx]
            VGS = VGS[idx]
  
        if (np.size(pars['NS']) == 1):
            ratio.shape += (1,)
        else:
    ...
    def gamma(self, **kwargs):
        ...
        Output:
            output: interpolated data specified by outkeys. Squeezed to remove extra
                    dimensions
        """   
        # should provide a GMID, VDS and NS
